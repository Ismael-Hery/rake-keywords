...
var frenchLinking = ['de','du','des'];

// Utility private function
function addCandidatesAndComponents(block, result) {
  var trimmed = block.asString.toLowerCase().trim();

  // remove french linking words possibly at the end
  if(frenchLinking.indexOf(block.asArray.slice(-1)) !== -1){
    block.asArray = block.asArray.slice(0,-1);
    console.log(block);
  }
